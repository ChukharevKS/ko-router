...
//			console.log(bindingContext,parent,page,parent.path())
						var randomTemplateName= "tmpl" + Math.round(Math.random() * 10000000)
//			var randomTemplateName = "tmpl-" + value.route
			if (!value.src) {
				var tmpl = document.createElement('div');
				tmpl.innerHTML =  "<script type='text/html' id='" + randomTemplateName + "'>" + element.innerHTML + "</script>";
			...
			console.log(parent,page.fullRoute())
			//			console.log("data.template", value.template, value.src, "tmpl-" + value.src)
			page.close();
			return {
...
//			console.log(route,page)
	...
	Page.prototype.ensureTemplate = function ensureTemplate() {
		...
		if (!tmpl && this.src) return ctx.fetch(encodeURI(this.src)).then(function(response) {
			...
			if (text.toLowerCase().substr(0, 7) !== "<script") {
				text = "<script type='text/html' id='" + tmplname + "'>" + text + "</script>"
			}
			var tmpl = document.createElement('div');
			tmpl.innerHTML = text;
	...
	function workGuard(cur) {
		...
		if (!next) return cur.page.closeChildren().then(function() {
			if (r) return Promise.reject(new Error('Page not found')); // непонятно
